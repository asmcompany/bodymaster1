...
var pandora = {
	...
	initHomepageSlider: function() {
		...
		$('#hompage-slider_content').touchwipe({
		    wipeRight: function() { $('#hompage-slider_content').cycle('next'); },
		    wipeLeft: function() { $('#hompage-slider_content').cycle('prev'); },
	...
	adjustSliderNavPos: function() {
		...
		nav.css('right', (scr_width-nav_width)/2);
	},
	...
	adjustQuickShopPopupPosition: function() {
		$('.quick-shop').css({
			position: 'fixed',
			right: ($(window).width() - $('.quick-shop').outerWidth())/2,
